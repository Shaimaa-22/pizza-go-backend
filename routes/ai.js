...
router.post("/ask", auth, async (req, res) => {
  ...
  if (message.includes("hello") || message.includes("hi")) {
    return res.json({
      reply: "Hi! Welcome to Pizza Go. You can tell me what toppings you want.",
      actions: [],
    });
  }
  ...
  if (message.includes("add") || message.includes("want") || message.includes("with")) {
    toppings.forEach((topping) => {
      if (message.includes(topping)) {
        actions.push({ type: "add_topping", topping });
      }
    });
    ...
    return res.json({
      reply: "This topping is not available right now. You can choose olives, tomato, or mushroom.",
      actions: [],
    });
  }

  if (message.includes("remove") || message.includes("without") || message.includes("no ")) {
    toppings.forEach((topping) => {
      if (message.includes(topping)) {
        actions.push({ type: "remove_topping", topping });
      }
    });
  ...
  return res.json({
    reply: "Sorry, I can help you add toppings, remove toppings, or continue to payment.",
    actions: [],
  });
});
